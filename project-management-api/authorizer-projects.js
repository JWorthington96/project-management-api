...
const generatePolicy = (user, projects, decodedToken) => {
    ...
    projects.map((project) => {
        ...
        if (project.projectManager === user.Username)
            managerProjectIds.push(id);
        else if (project.developers.includes(user.Username))
            developerProjectIds.push(id);
    });
    console.log(admin);
...
export function main(event, context, callback) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const user = yield cognito.call('getUser', {
                AccessToken: accessToken
            });
            console.log(user);
            const projects = yield dynamodb.call('scan', params);
