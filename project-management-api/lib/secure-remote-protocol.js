...
export function bigPrime() {
    ...
    while (!q.isProbablePrime() && !N.isProbablePrime() && r < 105749
        && N.bitLength().greaterOrEquals(26) && N.bitLength().lesserOrEquals(28))
        q.next();
    console.log(q);
    console.log(N);
...
export function getGenerator() {
    ...
    let primeFactors = [];
    let booleans = [];
    for (let a = 2; a < N.prev().toJSNumber(); a++)
        booleans.push(true);
    ...
    for (let i = 2; i < N.prev().toJSNumber(); i++) {
        if (booleans[i] && N.prev().isDivisibleBy(i))
            primeFactors.push(i);
    }
...
//# sourceMappingURL=secure-remote-protocol.js.map